...
public class HWSwiftyViewPager: UICollectionView, UICollectionViewDelegate {
    ...
    override public func layoutSubviews() {
        ...
        if !CGRectEqualToRect(self.frame, self.beforeFrame) {
            ...
            if numberOfItemsInSection(0) == 1 {
                itemWidth = frame.size.width - flowLayout.sectionInset.left - flowLayout.minimumLineSpacing
                itemHeigh = frame.size.height - flowLayout.sectionInset.top - flowLayout.sectionInset.bottom
            } else {
                itemWidth = frame.size.width - flowLayout.sectionInset.left * 2 - flowLayout.minimumLineSpacing
                itemHeigh = frame.size.height - flowLayout.sectionInset.top - flowLayout.sectionInset.bottom
            }
            
            flowLayout.itemSize = CGSize(width: itemWidth, height: itemHeigh)
            
            beforeFrame = frame
            
            itemWidthWithMargin = itemWidth + flowLayout.minimumLineSpacing
